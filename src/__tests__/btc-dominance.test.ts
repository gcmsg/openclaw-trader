/**
 * BTC dominance trend tracking tests
 *
 * Covers: trackBtcDominance / getBtcDominanceTrend / getLatestDominance
 * All file I/O is mocked via vi.spyOn(fs, ...), no real disk reads/writes.
 */
...
/** Build N consecutive records (starting from startDate, N days forward, dom changes linearly) */
...
describe("trackBtcDominance", () => {
  ...
  it("calls writeFileSync when writing a new record", () => {
  ...
  it("overwrites old record on same day (keeps only the latest)", () => {
  ...
  it("truncates old records beyond 30 days", () => {
    // Create 40 historical records
    ...
    // MAX_DAYS = 30, plus today's new record, total should be <= 30
    expect(saved.length).toBeLessThanOrEqual(30);
  });

  it("starts from an empty array when file does not exist", () => {
    vi.spyOn(fs, "existsSync").mockReturnValue(false);
    vi.spyOn(fs, "mkdirSync").mockImplementation(() => undefined);
    const writeSpy = vi.spyOn(fs, "writeFileSync").mockImplementation(() => undefined);
...
describe("getBtcDominanceTrend", () => {
  ...
  it("returns direction=neutral and latest=NaN when no history", () => {
    vi.spyOn(fs, "existsSync").mockReturnValue(false);
  ...
  it("dominance increase > 0.5% → direction=rising", () => {
    // 7 days ago 52%, today 54%, change +2%
  ...
  it("dominance decrease > 0.5% → direction=falling", () => {
    // 7 days ago 56%, today 54%, change -2%
  ...
  it("change < neutralThreshold → direction=neutral", () => {
    // 7-day change only +0.2%, below default threshold 0.5%
  ...
  it("custom neutralThreshold works", () => {
    // Change +0.8%, default threshold 0.5% would trigger rising, but custom 1.0% does not
  ...
  it("returned latest matches the last record", () => {
...
describe("getLatestDominance", () => {
  ...
  it("returns the last record when records exist", () => {
    const records: DominanceRecord[] = [
  ...
  it("returns undefined on corrupted file (does not throw)", () => {
    vi.spyOn(fs, "existsSync").mockReturnValue(true);
