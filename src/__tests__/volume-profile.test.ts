...
describe("calcVolumeProfile", () => {
  ...
  it("POC is within the price range", () => {
    const klines = makeKlines([
      ...
      { h: 115, l: 95, c: 105, v: 3000 }, // high volume
  ...
  it("empty klines returns zero values", () => {
...
describe("calcPivotPoints", () => {
  it("calculates PP based on the second-to-last kline", () => {
    const klines = makeKlines([
      { h: 120, l: 80, c: 100 },  // second-to-last
      { h: 115, l: 95, c: 110 },  // latest (not used for calculation)
    ]);
...
describe("calcSupportResistance", () => {
  it("support is below current price, resistance is above current price", () => {
    const klines = makeKlines(
  ...
  it("distanceToSupport and distanceToResistance are positive", () => {
    const klines = makeKlines(
      Array.from({ length: 30 }, (_, i) => ({
        h: 105 + i,
        l: 95 + i,
