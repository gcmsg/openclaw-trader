...
describe("analyzeSentimentWithLLM", () => {
  ...
  it("returns parsed result on successful LLM response", async () => {
    const mockLLMResponse = JSON.stringify({
      score: 6,
      label: "bullish",
      bullishReasons: ["ETF approved", "Price at new high"],
      bearishReasons: ["FED uncertainty"],
      confidence: "high",
      summary: "Overall bullish, institutional inflows evident",
    });
  ...
  it("extracts JSON even when LLM wraps it in prose", async () => {
    const wrapped = `Here is my analysis:\n${JSON.stringify({ score: -3, label: "bearish", bullishReasons: [], bearishReasons: ["Market declining"], confidence: "medium", summary: "Leaning bearish" })}\nThis concludes my analysis.`;
    mockedAxios.post = vi.fn().mockResolvedValue({
...
describe("llmResultToEntry", () => {
  it("converts LLM result to SentimentEntry shape", () => {
    const llmResult = {
      ...
      bullishReasons: ["Institutional buying"],
      bearishReasons: [],
      confidence: "medium" as const,
      summary: "Leaning bullish",
    };
...
describe("formatLLMSentimentReport", () => {
  it("formats report with all sections", () => {
    const result = {
      score: 7,
      label: "bullish" as const,
      bullishReasons: ["ETF approved", "Institutional buying"],
      bearishReasons: ["Regulatory risk"],
      confidence: "high" as const,
      summary: "Overall market leaning bullish",
    };
    ...
    expect(report).toContain("Overall market leaning bullish");
  });
