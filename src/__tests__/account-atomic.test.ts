/**
 * Bug 5: File I/O atomic write test
 *
 * Verify that saveAccount uses atomic writes (write to .tmp first, then rename),
 * preventing data corruption from concurrent writes.
 */
...
describe("saveAccount — atomic write", () => {
  it("file exists after save with no .tmp residue", () => {
    const id = uniqueId();
  ...
  it("data loaded back after save is consistent", () => {
    const id = uniqueId();
  ...
  it("two consecutive saves produce no .tmp residue", () => {
    const id = uniqueId();
  ...
  it("written file content is valid JSON", () => {
    const id = uniqueId();
  ...
  it("different scenarioId writes to different files", () => {
    const id1 = uniqueId();
    const id2 = uniqueId();
