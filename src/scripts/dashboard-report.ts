/**
 * Static HTML Dashboard Generator (CLI entry script)
 *
 * Reads paper account data → generates reports/dashboard.html
 * Includes: equity curve, max drawdown, trade records table, per-symbol stats
 *
 * Usage: npm run dashboard:report
 */
...
function buildDashboardData(scenarioId: string, initialUsdt: number): DashboardData {
  ...
  // ── Equity curve (rebuilt trade by trade) ──
  ...
  curve.push({
    ...
    label: new Date(account.createdAt).toLocaleDateString("en-US"),
    equity: initialUsdt,
    drawdown: 0,
  });
  ...
  for (const trade of trades) {
    ...
    curve.push({
      ts: trade.timestamp,
      label: new Date(trade.timestamp).toLocaleDateString("en-US"),
  ...
  const recentTrades: TradeRow[] = trades.slice(-50).reverse().map((t) => ({
    id: t.id,
    ts: new Date(t.timestamp).toLocaleString("en-US"),
    symbol: t.symbol,
    side: t.side,
    qty: t.quantity,
    price: t.price,
    usdt: t.usdtAmount,
    pnl: t.pnl ?? null,
    // PaperTrade.pnlPercent stored as ratio (0.038 = +3.8%), multiply by 100 for display
...
function generateHtml(datasets: DashboardData[]): string {
  ...
  // Serialize datasets for JS (JSON.stringify embedded in <script>)
...
<body>
  ...
  <p class="subtitle">Paper Trading — Generated ${genTime} | AI Agent 🤖</p>
  ...
  <div class="chart-row">
    <div class="card">
      <p class="section-title"><span class="dot"></span>Equity Curve</p>
      <div class="chart-wrap"><canvas id="equity-chart"></canvas></div>
    </div>
    <div class="card">
      <p class="section-title"><span class="dot"></span>Max Drawdown</p>
  ...
  <div class="card" style="margin-bottom:12px">
    <p class="section-title"><span class="dot"></span>Per-Symbol Statistics</p>
    <div id="symbol-stats-wrap"></div>
  </div>

  <!-- Recent Trades -->
  <div class="card" style="margin-bottom:12px">
    <p class="section-title"><span class="dot"></span>Last 50 Trades</p>
    <div id="trades-wrap" style="overflow-x:auto"></div>
  </div>

  <div class="footer">openclaw-trader • paper mode • data for reference only • all operations require user authorization</div>
...
function buildKpi(d) {
  ...
  el.innerHTML = [
    { label: 'Current Equity', val: '$' + fmt(d.currentEquity), cls: 'neutral' },
    { label: 'Total Return', val: (d.totalReturn >= 0 ? '+' : '') + fmt(d.totalReturn) + '%', cls: rClass },
    { label: 'Realized PnL', val: (d.totalPnl >= 0 ? '+$' : '-$') + fmt(Math.abs(d.totalPnl)), cls: rClass },
    { label: 'Trades', val: d.numTrades + '', cls: 'neutral' },
    { label: 'Win Rate', val: fmt(d.winRate * 100) + '%', cls: wClass },
    { label: 'Wins / Losses', val: d.numWins + ' / ' + (d.numTrades - d.numWins), cls: 'neutral' },
    { label: 'Max Drawdown', val: fmt(d.maxDrawdown) + '%', cls: ddClass },
    { label: 'Initial Capital', val: '$' + fmt(d.initialUsdt), cls: 'neutral' },
...
function buildSymbolStats(d) {
  ...
  if (!d.symbolStats.length) {
    wrap.innerHTML = '<div class="empty-msg">No trade records yet</div>';
    return;
  }
  wrap.innerHTML = '<table><thead><tr>' +
    '<th>Symbol</th><th>Trades</th><th>Win Rate</th><th>Total PnL</th><th>Avg PnL</th>' +
    '</tr></thead><tbody>' +
...
function buildTrades(d) {
  ...
  wrap.innerHTML = '<table><thead><tr>' +
    '<th>Time</th><th>Symbol</th><th>Side</th><th>Qty</th><th>Price</th><th>Amount(U)</th><th>PnL</th><th>PnL%</th>' +
    '</tr></thead><tbody>' +
...
function main() {
  ...
  for (const cfg of configs) {
    try {
      ...
      console.warn(`⚠️ Skipping scenario ${cfg.paper.scenarioId}: ${e instanceof Error ? e.message : String(e)}`);
    }
  }

  if (datasets.length === 0) {
    // Create empty dashboard (can still generate when no trade records exist)
    datasets.push({
      scenarioId: "default",
      generatedAt: new Date().toLocaleString("en-US"),
      initialUsdt: 1000,
      ...
      winRate: 0,
      maxDrawdown: 0,
      equityCurve: [{ ts: Date.now(), label: new Date().toLocaleDateString("en-US"), equity: 1000, drawdown: 0 }],
      recentTrades: [],
      symbolStats: [],
    });
  }
  ...
  console.log(`   Scenarios: ${datasets.map((d) => d.scenarioId).join(", ")}`);
  console.log(`   Total trades: ${datasets.reduce((s, d) => s + d.numTrades, 0)}`);
}
...
try {
  ...
  console.error("Dashboard generation failed:", e instanceof Error ? e.message : String(e));
  process.exit(1);
}
