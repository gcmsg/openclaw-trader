 ...
 * Uses pure SVG templates to generate charts (no native canvas dependency).
 * Also provides ASCII text version for terminal output.
 */
...
/**
 * Generate equity curve SVG chart.
 * @param equityPoints Time series [{ timestamp, equity }]
 * @param title Chart title
 * @param outputPath Output path (.svg extension)
...
/** Generate ASCII text chart (for Telegram messages) */
...
function buildSvg(
  ...
  if (points.length > 0) {
    ...
    for (let i = 0; i < labelCount; i++) {
      ...
      const label = new Date(pt.timestamp).toLocaleDateString("en-US", {
        month: "short",
        day: "numeric",
      });
      xLabels.push(
