...
export interface ScenarioStats {
  ...
  initialUsdt: number;
  currentUsdt: number;
  totalEquity: number; // includes unrealized position PnL (calculated with live prices)
  totalPnl: number;
  totalPnlPct: number;
...
// ─────────────────────────────────────────────────────
// Data collection (requires live prices)
// ─────────────────────────────────────────────────────
...
async function collectStats(): Promise<ScenarioStats[]> {
  ...
  // First scan all scenarios and collect symbols with open positions
  ...
  // Batch fetch live prices (single API call)
  ...
  return paperCfg.scenarios.map((scenario) => {
    ...
    const leverageLabel = lev?.enabled ? `${lev.default}x` : "N/A";
    ...
    const account = loadAccount(scenario.initial_usdt, scenario.id);
    // Calculate total equity using live prices (positions valued at current market price)
...
function printCompareReport(stats: ScenarioStats[]): void {
  ...
  console.log(`\n${"═".repeat(72)}`);
  console.log(`  📊 Multi-Strategy Comparison Report  ${now}`);
  ...
  for (const [i, s] of sorted.entries()) {
    ...
    console.log(`${rankEmoji}  ${s.name}${!s.hasData ? " [no data]" : ""}`);
    console.log(`    Strategy: ${s.strategyName}  |  Market: ${s.market}  |  Leverage: ${s.leverage}`);
    console.log(
      `    ${pnlEmoji} Total PnL: ${pnlSign}$${fmt(s.totalPnl)}  (${pnlSign}${fmt(s.totalPnlPct)}%)`
    );
    console.log(`    ${bar(s.totalPnlPct)} ${fmt(s.totalPnlPct, 1, true)}%`);

    if (s.hasData && s.totalTrades > 0) {
      console.log(`    💼 Total Equity: $${fmt(s.totalEquity)} | Positions: ${s.openPositions}`);
      console.log(
        `    📈 Trades: ${s.totalTrades} | Wins: ${s.winTrades} | Losses: ${s.lossTrades} | Win Rate: ${fmt(s.winRate * 100, 1)}%`
      );
      if (s.winTrades > 0 || s.lossTrades > 0) {
        console.log(
          `    🎯 Avg Win: ${fmt(s.avgWinPct, 1, true)}%  Avg Loss: ${fmt(s.avgLossPct, 1)}%  Max Win: ${fmt(s.maxSingleWinPct, 1, true)}%  Max Loss: ${fmt(s.maxSingleLossPct, 1)}%`
        );
      }
    } else if (!s.hasData) {
      console.log(`    ℹ️  Account not yet created (waiting for first trade signal)`);
    }
    console.log();
  }
  ...
  // Quick summary table
  console.log("─".repeat(72));
  console.log(`  Scenario                  Return   Win Rate  Trades  Pos.`);
  console.log("─".repeat(72));
...
// ─────────────────────────────────────────────────────
// Exports (used by report/weekly.ts)
// ─────────────────────────────────────────────────────
...
export function formatCompareReport(stats: ScenarioStats[]): string {
  const now = new Date().toLocaleString("en-US");
  const sorted = [...stats].sort((a, b) => b.totalPnlPct - a.totalPnlPct);
  const lines: string[] = [`📊 **Multi-Strategy Comparison Report** ${now}`, ``];
  ...
  for (const [i, s] of sorted.entries()) {
    ...
    lines.push(
      `  Return: ${pnlSign}${fmt(s.totalPnlPct, 2)}%  Win Rate: ${s.totalTrades > 0 ? fmt(s.winRate * 100, 0) + "%" : "--"}  Trades: ${s.totalTrades}`
    );
  }
